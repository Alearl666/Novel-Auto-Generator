    ...
    <div class="ttw-prompt-section">
        ...
        <div id="ttw-suffix-content" class="ttw-prompt-content">
            ...
            <div style="border-top:1px solid rgba(255,255,255,0.1);padding-top:12px;">
                ...
                <div id="ttw-preset-name-hint" style="margin-top:6px;font-size:11px;color:#9b59b6;"></div>
            </div>
        </div>
    </div>`;
}
...
export function hydrateSettingsFromState(deps = {}) {
    ...
    const temperatureEl = document.getElementById('ttw-temperature');
    if (temperatureEl) {
        const t = AppState.settings.presetTemperature;
        temperatureEl.value = typeof t === 'number' && !isNaN(t) ? t : 0.3;
    }

    const maxTokensEl = document.getElementById('ttw-max-tokens');
    if (maxTokensEl) {
        const mt = AppState.settings.presetMaxTokens;
        maxTokensEl.value = typeof mt === 'number' && mt > 0 ? mt : '';
    }

    const presetHintEl = document.getElementById('ttw-preset-name-hint');
    if (presetHintEl) {
        const name = AppState.settings.importedPresetName;
        presetHintEl.textContent = name ? `📥 已导入预设：${name}` : '';
    }
